...
export const CartProvider = ({ children }) => {
  ...
  const [userDataOut, setUserDataOut] = useState(
  ...
  const addToCart = (service) => {
    //agregar al carro
    setCart((prevCart) => {
      const existing = prevCart.find((p) => p.id === service.id);
      if (existing) {
        return prevCart.map(
          (p) => (p.id === service.id ? { ...p, cantidad: p.cantidad + 1 } : p) // si existe la suma al total
        );
      } else {
        return [...prevCart, { ...service, cantidad: 1 }]; //si no existe la agrega
      }
    });
  };

  const total = cart.reduce(
    (acc, service) => acc + service.price * service.cantidad,
    0
  ); // calculamos el total
  const itemCount = cart.reduce((acc, service) => acc + service.cantidad, 0); // contamos las services
  ...
  const handlePay = async () => {
    try {
      ...
      const shortCart = cart.map((service) => ({
        id: service.id,
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const data = { Nombre: nombre, Apellido: apellido, Email: userEmail, Telefono: telefono, Contraseña: contraseña };
      const url = "https://webkinesara-back.onrender.com/usuarios";
